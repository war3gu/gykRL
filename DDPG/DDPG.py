...
if __name__=='__main__':
    ...
    while True:
        ...
        action = actor.act(state1, oun())
        next_state, reward, done, info = env.step(action[0])

        replayMemory.add(state, action[0], reward, done, next_state, None)

        state = next_state

        episode_reward += reward

        if replayMemory.size() > int(64):
            ...
            grads = critic.action_gradients(state_b, actions_pred)  # 算出Q对action的梯度
